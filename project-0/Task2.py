...
def total_call_duration_dict():
    ...
    """
    call_durations = {}
    for outgoing, call in groupby(calls, key=itemgetter(0)):
        if outgoing not in call_durations:
            call_durations[outgoing] = []
        else:
            call_durations[outgoing] += list(call)

    for incoming, call in groupby(calls, key=itemgetter(1)):
    ...
    for number, calls_for_number in call_durations.items():
        durations = [int(call[3]) for call in calls_for_number]
        total_call_durations[number] = sum(durations)

    return total_call_durations


if __name__ == '__main__':
    total_durations = total_call_duration_dict()
